...
/**
 * ================================================================= 
 * Source file taken from :: staticappprops.js
 * ================================================================= 
 */
...
/* This file is for Windows Universal only. We need to read the mfpclient.properties file 
 * and load the WL.StaticAppProps variable before worklight.js is loaded 
 */
function __loadStaticAppProps(lines){
    var props = {};
    for (var l = 0 ; l < lines.length ; l++) {
        var trim = lines[l].trim();
        if (trim.charAt(0) !== '#' && trim.length > 0) {
            var property = trim.split('=', 2);
            if (property.length > 1) {
                props[property[0].trim()] = property[1].trim();
            }
        }
    }
    var context = props['wlServerContext'];
    if (context.charAt(context.length - 1) === '/') {
        context = context.substring(0, context.length - 1);
    }
    var rootUrl = props['wlServerProtocol'] + '://' + props['wlServerHost'] + ':' + props['wlServerPort'] + context;
    
    WL.StaticAppProps.APP_SERVICES_URL = rootUrl + WL.StaticAppProps.APP_SERVICES_URL;
    if (window.clientInformation.userAgent.indexOf('Windows NT 10.0') > -1 || window.clientInformation.userAgent.indexOf('Windows Phone 10.0') > -1) {
        WL.StaticAppProps.WORKLIGHT_ROOT_URL = rootUrl + WL.StaticAppProps.WORKLIGHT_ROOT_URL_WIN;
        WL.StaticAppProps.APP_VERSION = WL.StaticAppProps.APP_VERSION_WIN;
        WL.StaticAppProps.ENVIRONMENT = WL.StaticAppProps.ENVIRONMENT_WIN;
        }
    else if (window.clientInformation.userAgent.indexOf('Windows Phone 8.1') > -1) {
        WL.StaticAppProps.WORKLIGHT_ROOT_URL = rootUrl + WL.StaticAppProps.WORKLIGHT_ROOT_URL_WP8;
        WL.StaticAppProps.APP_VERSION = WL.StaticAppProps.APP_VERSION_WP8;
        WL.StaticAppProps.ENVIRONMENT = WL.StaticAppProps.ENVIRONMENT_WP8;
        }
    else {
        WL.StaticAppProps.WORKLIGHT_ROOT_URL = rootUrl + WL.StaticAppProps.WORKLIGHT_ROOT_URL;
    }
}
try
{
var url = new Windows.Foundation.Uri('ms-appx:///mfpclient.properties');
Windows.Storage.StorageFile.getFileFromApplicationUriAsync(url).then(function (file) {
    Windows.Storage.FileIO.readLinesAsync(file)
    .then(function (lines) {
        __loadStaticAppProps(lines);
    },
    function (err){
		console.error('Unable to read mfpclient.properties. Ensure that mfpclient.properties has the right permissions. ' + JSON.stringify(err));
    });
},
function(err){
	console.error('Unable to load mfpclient.properties. Ensure that mfpclient.properties is available.' + JSON.stringify(err));
}
);
}
catch(err)
{
    console.log('Unable to load mfpclient.properties. Ensure that mfpclient.properties is available.' + JSON.stringify(err));
}
...
/**
 * ================================================================= 
 * Source file taken from :: deviceAuthenticationUniversal.js
 * ================================================================= 
 */
...
__WLDeviceAuth = function() {
    ...
    this.init = function (initCallback) {
        WL.DeviceAuth.__initDeviceAuthManager(function () { initCallback(); });
    },
    ...
    this.__isCertificateExists = function (provisioningEntitiy, successCallback, failureCallback) {
        ...
        WLWin8Native.DeviceAuthPlugin.isCertificateExists(JSON.stringify(provisioningEntitiy)).done(
		        function completed(result) {
		            if (result.isSuccess) {
		                successCallback(JSON.parse(result.value));
		                dfd.resolve();
		            } else {
		                failureCallback(JSON.parse(result.value));
		                WL.Logger.error("Error: " + JSON.parse(result.value)); //TODO
		                dfd.reject();
		            }
		        }
			);
    ...
    this.__clearDeviceCertificate = function (provisioningEntitiy, successCallback, failureCallback) {
        ...
        var dfd = WLJQ.Deferred();
        WLWin8Native.DeviceAuthPlugin.clearDeviceCertCredentials(JSON.stringify(provisioningEntitiy)).done(
		        function completed(result) {
		            if (result.isSuccess) {
		                dfd.resolve();
		            } else {
    ...
    this.signDeviceAuth = function (successCallback, failureCallback, payloadJSON, provisioningEntitiy, isProvisioningEnabled) {
        ...
        var dfd = WLJQ.Deferred();
        WLWin8Native.DeviceAuthPlugin.signDeviceAuth(JSON.stringify(payloadJSON), JSON.stringify(provisioningEntitiy), JSON.stringify(isProvisioningEnabled)).done(
		        function completed(result) {
		            if (result.isSuccess) {
		                successCallback(JSON.parse(result.value));
		                dfd.resolve();
    ...
    this.saveCertificate = function (successCallback, failureCallback, provisioningEntitiy, certificate, realm) {
        ...
        var dfd = WLJQ.Deferred();
        WLWin8Native.DeviceAuthPlugin.saveCertificate(JSON.stringify(provisioningEntitiy), JSON.stringify(certificate), JSON.stringify(realm)).done(
		        function completed(result) {
		            if (result.isSuccess) {
		                successCallback();
		                dfd.resolve();
    ...
    this.signCsr = function (csrData, provisioningEntitiy, successCallback, failureCallback) {
        ...
        var dfd = WLJQ.Deferred();
        WLWin8Native.DeviceAuthPlugin.signCsr(JSON.stringify(csrData), JSON.stringify(provisioningEntitiy)).done(
		        function completed(result) {
		           if (result.isSuccess) {
		                successCallback(JSON.parse(result.value));
		                dfd.resolve();
    ...
    this.__initDeviceAuthManager = function (initCallback) {
        ...
        WLWin8Native.DeviceAuthPlugin.init().done(
                    function completed(result) {
                        if (result.isSuccess) {
                            initCallback();
                        } else {
    ...
    this.__getDeviceDisplayName = function(successCallback, failureCallback) {
       ...
       WLAuthorizationManager.__invokeGetRegistrationDataRequest(successCallback, failureCallback);
    },
    ...
    this.__setDeviceDisplayName = function(deviceDisplayName, successCallback, failureCallback) {
       ...
       WLAuthorizationManager.__setDeviceDisplayName(deviceDisplayName, successCallback, failureCallback);
    };
};
...
/**
 * ================================================================= 
 * Source file taken from :: wlgap.windows8.js
 * ================================================================= 
 */
...
__WLOptionsMenu = function () {
    ...
    this.init = function (options) {
        ...
        WLJSX.append(WLJSX.$$$('body'), wlAppBar);
    ...
    this.isEnabled = function () {
        ...
        // calls Windows8 code. returning the oposite value since IBM MobileFirst Platform API is asking for if it is 
...
WL.Device.setDeviceDisplayName = function(deviceDisplayName, options){
    WL.Client.__setDeviceDisplayName(deviceDisplayName, options);
};

WL.Device.getDeviceDisplayName = function(options){
    WL.Client.__getDeviceDisplayName(options);
};

WL.Device.getHardwareIdentifier = function(){
...
WL.App.__hashData = function(data, someArgs, callback) {
	...
	var browser = WLJSX.detectBrowser();
	if (browser.isWP81RT) { //Universal apps
		win8WinRtLib = new WLWin8Native.WPAppAuthPlugin();
	} else {
...
WL.App.__SecureRandom = function(successCallback,failureCallback){
    win8WinRtLib = new WLWin8Native.SecureRandom();
    win8WinRtLib.srandom("32").done(
            function completed(result){
                if (result.isSuccess){
                    successCallback(JSON.parse(result.value));
                    } else {
...
/**
// * Webresource checksum validation for the windows8/windowsphone8 universal environment.
 */

WL.App.__webResourceChecksumCheck = function (callback) {
    WLWin8Native.ChecksumPlugin().calculateChecksum("").done(

    //When the result has completed, check the status.
    function completed(result) {
        if (result.isSuccess) {
            res = JSON.parse(result.value);
            var checksumValid = JSON.parse(result.value).Result;
            if (checksumValid === false) {
                WL.Logger.error("Application failed to load, because of checksum mismatch. This may indicate unintended change to the application.");
                WL.SimpleDialog.show("Checksum validation failed", "Application failed to load, because of checksum mismatch. This may indicate unintended change to the application.", [{
                    text: WL.ClientMessages.ok,
                    handler: function () {
                        window.close();
                    }
...
WLAuthorizationManager.__sendLogoutRequest = function (securityCheck) {
    var dfd = WLJQ.Deferred();
    WLAuthorizationManager.clientId = WLAuthorizationManager.__getFromAppSetting(WLAuthorizationManager.CLIENT_ID_OAUTH_LABEL);
    if (WLAuthorizationManager.clientId == null || typeof (WLAuthorizationManager.clientId) === 'undefined') {
        WL.Logger.debug('Cannot logout before client is registered.');
        dfd.reject(WLAuthorizationManager.__createErrorResponse(500, WL.ErrorCode.AUTHORIZATION_FAILURE, 'Cannot logout before client is registered.'));
    }
    else {
        WLAuthorizationManager.__getWlServerUrl().then(
          function (serverUrl) {
              ...
              var jwt = WLAuthorizationManager.__createJwt(null, null);
              WLAuthorizationManager.__signJWS(jwt, WLAuthorizationManager.clientId).then(
                function (jws) {
                    var params = [];
                    params.push(encodeURIComponent('security_check') + "=" + encodeURIComponent(securityCheck));
                    params.push(encodeURIComponent('client_assertion') + "=" + encodeURIComponent(jws['header'] + "." + jws['payload'] + "." + jws['signature']));
                    params.push(encodeURIComponent('client_assertion_type') + "=" + encodeURIComponent(WLAuthorizationManager.PARAM_CLIENT_ASSERTION_TYPE_VALUE));
                    params = params.join("&");
                    new WLJSX.Ajax.WLRequest(serverUrl + WLAuthorizationManager.LOGOUT_PATH, {
                        method: 'POST',
                        parameters: params,
                        optionalHeaders: {
                        ...
                        onFailure: function (error) {
                            WL.Logger.debug('Logout unsuccessful');
                            dfd.reject(error);
                        }
                    });
                },
                    function (error) {
                        WL.Logger.debug('Failed to sign the JWS:' + JWSData);
                        dfd.reject(error);
                    }
                    );
          },
...
WLAuthorizationManager.__createErrorResponse= function(status, errorCode, errorMsg) {
    var transport = {
        status: status,
        responseJSON: {
            errorCode: errorCode,
...
WLAuthorizationManager.__setDeviceDisplayName = function(deviceDisplayName, successCallback, failureCallback) {
  WLAuthorizationManager.deviceDisplayName = deviceDisplayName;
  WLAuthorizationManager.clientId = WLAuthorizationManager.__getFromAppSetting(WLAuthorizationManager.CLIENT_ID_OAUTH_LABEL);
  if (WLAuthorizationManager.clientId != null || typeof ((WLAuthorizationManager.clientId) != 'undefined') ){
  WLAuthorizationManager.__invokeRegistrationRequest()
  .then(
    function (response) {
      WLAuthorizationManager.deviceDisplayName = null;
      successCallback();
    },
    function (error) {
      WLAuthorizationManager.deviceDisplayName = null;   
   ...
   WLAuthorizationManager.deviceDisplayName = null;   
   failureCallback('set device friendly name is allowed only after regisration');
  }  
}

WLAuthorizationManager.__invokeGetRegistrationDataRequest = function (successCallback, failureCallback) {
  WLAuthorizationManager.clientId = WLAuthorizationManager.__getFromAppSetting(WLAuthorizationManager.CLIENT_ID_OAUTH_LABEL);
    if (WLAuthorizationManager.clientId == null || typeof (WLAuthorizationManager.clientId) === 'undefined') {
        WL.Logger.debug('application is not registered yet');
        failureCallback("get device display name failed");
    }
    else {
        WLAuthorizationManager.__getWlServerUrl().then(
          function (serverUrl) {
              ...
              WLAuthorizationManager.__signJWS(jwt, WLAuthorizationManager.clientId).then(
                function (jws) {
                    ...
                    params.push(encodeURIComponent('client_assertion_type') + "=" + encodeURIComponent(WLAuthorizationManager.PARAM_CLIENT_ASSERTION_TYPE_VALUE));
                    params = params.join("&");
                    new WLJSX.Ajax.WLRequest(serverUrl + WLAuthorizationManager.REGISTRATION_PATH + '/' + WLAuthorizationManager.clientId, {
                        method: 'GET',
                        parameters: params,
                        optionalHeaders: {},
                        onSuccess: function (response) {
                            var deviceDisplayName = response.responseJSON.registration.device.deviceDisplayName;
                            successCallback(deviceDisplayName);
                        },
                        onFailure: function (error) {
                            WL.Logger.debug("get device display name failed");
                            failureCallback("get device display name failed with following error : " + error);
                        }
                    });
                },
                    function (error) {
...
WLAuthorizationManager.__sendHeartBeat = function (intervalInSecs) {
    WLAuthorizationManager.heartBeatIntervalInSecs = intervalInSecs;
    // stop heart beat on pause
    document.addEventListener("pause", function () {
        if (WLAuthorizationManager.heartBeatPeriodicalExecuter) {
            WLAuthorizationManager.heartBeatPeriodicalExecuter.stop();
        }
    }, false);
    // continue heart beat on resume
    document.addEventListener("resume", function () {
        if (WLAuthorizationManager.heartBeatPeriodicalExecuter) {
            WLAuthorizationManager.heartBeatPeriodicalExecuter = new WLJSX.PeriodicalExecuter(WLAuthorizationManager.__HeartBeatTask, WLAuthorizationManager.heartBeatIntervalInSecs);
        }
    }, false);
    if (WLAuthorizationManager.heartBeatPeriodicalExecuter) {
        WLAuthorizationManager.heartBeatPeriodicalExecuter.stop();
        WLAuthorizationManager.heartBeatPeriodicalExecuter = null;
    }

    if (WLAuthorizationManager.heartBeatIntervalInSecs > 0) {
        WLAuthorizationManager.heartBeatPeriodicalExecuter = new WLJSX.PeriodicalExecuter(WLAuthorizationManager.__HeartBeatTask, WLAuthorizationManager.heartBeatIntervalInSecs);
    }
}

WLAuthorizationManager.__HeartBeatTask = function () {
    WLAuthorizationManager.clientId = WLAuthorizationManager.__getFromAppSetting(WLAuthorizationManager.CLIENT_ID_OAUTH_LABEL);
    if (WLAuthorizationManager.clientId == null || typeof (WLAuthorizationManager.clientId) === 'undefined') {
          ...
          function (serverUrl) {
              ...
              WLAuthorizationManager.__signJWS(jwt, WLAuthorizationManager.clientId).then(
                function (jws) {
                    ...
                    params.push(encodeURIComponent('client_assertion_type') + "=" + encodeURIComponent(WLAuthorizationManager.PARAM_CLIENT_ASSERTION_TYPE_VALUE));
                    params = params.join("&");
                    new WLJSX.Ajax.WLRequest(serverUrl + WLAuthorizationManager.HEART_BEAT_PATH, {
                        method: 'POST',
                        parameters: params,
                        optionalHeaders: {
                            'Content-Type': 'application/x-www-form-urlencoded; charset=utf-8'
...
WLAuthorizationManager.__updateServerRelativeTime = function() {
  var dfd = WLJQ.Deferred();
  try {
      WLAuthorizationManager.serverRelativeTime = WLAuthorizationManager.__serverRelativeTime;
      dfd.resolve(); 
    }
    catch (ex) {
      dfd.reject(ex);
...
WLAuthorizationManager.__cacheScopeByResource = function(resource, scope) {
  var dfd = WLJQ.Deferred();
  try {
    var requestoptions = resource['requestOptions'];
    var okrequest = requestoptions['url']+'_'+requestoptions['method'];
    if (okrequest!= null && scope!= null && scope.trim().length != 0) {
      WLAuthorizationManager.resourceToScopeCache[okrequest] = this.__sortScope(scope);
      dfd.resolve(); 
    } }
    catch (ex) {
      dfd.reject(ex);
...
WLAuthorizationManager.clearAccessToken = function(accessToken) {
    var dfd = WLJQ.Deferred();
    ...
    try {
        ...
        delete WLAuthorizationManager.scopetotoken[WLAuthorizationManager.__sortScope(scope)]; 
        dfd.resolve();
    } 
    catch (ex) {
      WL.Logger.error('WLAuthorizationManager.clearAccessToken failed with error: ' + ex.message);
      dfd.reject(ex);
    } 
   return dfd.promise();  
}

WLAuthorizationManager.getAuthorizationServerUrl = function() {
  var dfd = WLJQ.Deferred();
  try {
    var url = (WLAuthorizationManager.authorizationServerUrl != null)? WLAuthorizationManager.authorizationServerUrl : WL.StaticAppProps.WORKLIGHT_ROOT_URL;
    dfd.resolve(url);
  }
  catch (ex) {
    WL.Logger.Error('Failed to create URL from server root path');
    dfd.reject(ex);
  }
 return dfd.promise();
}

WLAuthorizationManager.setAuthorizationServerUrl = function(url) {
  var dfd = WLJQ.Deferred();
  try {
    WLAuthorizationManager.authorizationServerUrl = url;
    dfd.resolve();
  }
  catch (ex) {
    dfd.reject(ex);
...
WLAuthorizationManager.__getCachedAccessToken = function(scope) {
  var dfd = WLJQ.Deferred();
  try {
    if (scope == null || scope == "") { 
    scope = 'default_scope';
    ...
    scope = this.__sortScope(scope);
    }
    var accessToken = WLAuthorizationManager.scopetotoken[scope];
    if (accessToken == null || typeof (accessToken) === 'undefined') {
      dfd.resolve(null); 
    }
    else if (accessToken['expireIn'] > new Date().getTime()) {
      var token = {};
      token['scope'] = scope;
      token['value'] = accessToken['value'];
      token['asAuthorizationRequestHeader'] = "Bearer " + accessToken['value'];
      token['asFormEncodedBodyParameter'] = "access_token=" + accessToken['value'];
      dfd.resolve(token);
    }
    else {
      delete WLAuthorizationManager.scopetotoken['scope'];
      dfd.resolve(null); 
    }
  }
  catch (ex) {
    WL.Logger.debug('getCachedAccessToken for scope ' + scope + ' failed: ' + ex.message);
    dfd.reject(ex);
  }
  return dfd.promise();
}
...
WLAuthorizationManager.__deleteAuthData = function() {
  var dfd = WLJQ.Deferred();
  try {
    this.__clearCachedResourcesAndToken();
    this.clientId = null;
    this.__storeInAppSetting(WLAuthorizationManager.CLIENT_ID_OAUTH_LABEL, null);
    this.__storeInAppSetting(WLAuthorizationManager.CLIENT_REGISTRATION_DATA_KEY, null); 
    this.__removeFromAppSetting(WLAuthorizationManager.PRIVATE_KEY);
    this.__removeFromAppSetting(WLAuthorizationManager.PUBLIC_KEY);  
    dfd.resolve(); 
    }
    catch (ex) {
      dfd.reject(ex);
...
WLAuthorizationManager.__getCachedScopeByResource = function(request) {
  var dfd = WLJQ.Deferred();
  try {
      var requestoptions = request['requestOptions'];
      var okrequest = requestoptions['url']+'_'+requestoptions['method'];
      if (okrequest!= null) {
        var scope = WLAuthorizationManager.resourceToScopeCache[okrequest];
        if (scope==null) {
          dfd.resolve("");
        } 
       dfd.resolve(scope); 
      ...
      WL.Logger.debug('getScopeByResource failed: ' + ex.message);
      dfd.reject(ex);
    } 
  return dfd.promise();
}

WLAuthorizationManager.__clearCachedResourcesAndToken = function() {
  WLAuthorizationManager.scopetotoken = {};
  WLAuthorizationManager.resourceToScopeCache = {};
}

WLAuthorizationManager.__invokeAuthorizationRequestForScope = function(scope) {
  var dfd = WLJQ.Deferred();
  WLAuthorizationManager.__invokeAuthorizationRequest(scope)
  .then(
    function (response) {
        WLAuthorizationManager.__getCachedAccessToken(scope).then(
            function (token) {
            dfd.resolve(token)});
    },
    function (error) {
...
WLAuthorizationManager.__invokeAuthorizationRequest = function(scope) {
  var dfd = WLJQ.Deferred();
  var params = {};
  params['response_type'] = 'code';
  params['client_id'] = this.clientId;
  params['redirect_uri'] = WLAuthorizationManager.PARAM_REDIRECT_URI_VALUE;
  params['scope'] = scope!=null? scope:"";
  params = WLJSX.Object.toQueryString(params);
        WLAuthorizationManager.__getWlServerUrl().then(
        function(serverUrl) {
          if (serverUrl[serverUrl.length - 1] !== '/') {
            serverUrl += '/';
          }
          serverUrl = (WLAuthorizationManager.authorizationServerUrl != null)? WLAuthorizationManager.authorizationServerUrl : serverUrl;
          new WLJSX.Ajax.WLRequest( serverUrl + WLAuthorizationManager.AUTHORIZATION_PATH, {
            method : 'GET',
            parameters : params,
            optionalHeaders: {},
            onSuccess : function(response) {
                WLAuthorizationManager.__onAuthorizationSuccess(scope, response).then(
                                            function (response) {
                                              try{
                                                WLAuthorizationManager.__onTokenSuccess(response);
                                                dfd.resolve(response);
                                              }
                                              catch(ex) {
                                                dfd.reject(ex);
...
WLAuthorizationManager.__onAuthorizationSuccess = function (scope, response) {
   var dfd = WLJQ.Deferred();
    var data = response.getHeader('Location');
    if (data != null ) {
          var parts = data.split('code=');
          var grantCode = parts[parts.length-1];
          if (grantCode == null || grantCode == 'undefined')
          { 
                WLAuthorizationManager.__onObtainAccessTokenFailure(scope,response);
                dfd.reject();
          } else {
                this.__InvokeTokenRequest(grantCode).then(
                      function (response) {
                        dfd.resolve(response);
                      },
                      function (error) {
          ...
          /* a valid response from the authorization end-point must include this header,
                 * if it's not present then something went wrong */
           WLAuthorizationManager.__onObtainAccessTokenFailure(scope,response);
           dfd.reject(response);
      }
       return dfd.promise();
...
WLAuthorizationManager.__AccessToken = function (jsonObject) {
    var accesstoken = {};
  var scopeFromJson = jsonObject.scope;
  accesstoken['scope'] = scopeFromJson? scopeFromJson:'default_scope';
    accesstoken['expTimeInMillis'] = (jsonObject.expires_in * 1000);
    accesstoken['expireIn'] = new Date().getTime() + accesstoken['expTimeInMillis'];
    accesstoken['value'] = jsonObject.access_token;
    return accesstoken;
}

WLAuthorizationManager.__sortScope = function (scope) {
  if (scope) {
    scope = scope.split('\\s+');
    scope = scope.sort();
...
WLAuthorizationManager.__saveTokenByScope = function (response) {
  try {
    var accesstoken = WLAuthorizationManager.__AccessToken(response);
        var scope = WLAuthorizationManager.__sortScope(accesstoken.scope);
    WLAuthorizationManager.scopetotoken[scope] = accesstoken;
  }
  catch (ex) {
    WL.Logger.debug('Could not save json response as token');
  }
}

WLAuthorizationManager.__createJwt = function (path, grantCode) {
  var DEFAULT_JWT_EXPIRATION = 60 * 1000;
    var exp =(new Date().getTime() + DEFAULT_JWT_EXPIRATION);
  var iat = new Date().getTime();
  var jwt = {};
  jwt['iss'] = (WL.StaticAppProps.APP_ID + '$' + WL.StaticAppProps.ENVIRONMENT + '$' + WL.StaticAppProps.APP_VERSION);
  jwt['sub'] = WLAuthorizationManager.clientId;
  jwt['aud'] = path;
  jwt['exp'] = WLAuthorizationManager.serverRelativeTime? (WLAuthorizationManager.serverRelativeTime + exp) : exp;
//jwt['nbf'] = null;
  jwt['iat'] = WLAuthorizationManager.serverRelativeTime? (WLAuthorizationManager.serverRelativeTime + iat) : iat;
  jwt['jti'] = grantCode;
  return jwt; 
}

WLAuthorizationManager.__InvokeTokenRequest = function(grantCode) {
  var dfd = WLJQ.Deferred();
  WLAuthorizationManager.__getWlServerUrl().then(
    function(serverUrl) {
     if (serverUrl[serverUrl.length - 1] !== '/') {
     ...
     var jwt = WLAuthorizationManager.__createJwt(serverUrl + WLAuthorizationManager.OAUTH_TOKEN_PATH + "/", grantCode);
    WLAuthorizationManager.__signJWS(jwt, WLAuthorizationManager.clientId).then(
      function (jws) {
          var params = [];
        params.push(encodeURIComponent('client_assertion') + "=" +  encodeURIComponent(jws['header']+"."+jws['payload']+"."+jws['signature']));
        params.push(encodeURIComponent('code') + "=" + encodeURIComponent(grantCode));
        params.push(encodeURIComponent('grant_type') + "=" + encodeURIComponent(WLAuthorizationManager.PARAM_AUTHORIZATION_CODE_VALUE));
        params.push(encodeURIComponent('redirect_uri') + "=" + encodeURIComponent(WLAuthorizationManager.PARAM_REDIRECT_URI_VALUE));
        params.push(encodeURIComponent('client_assertion_type') + "=" + encodeURIComponent(WLAuthorizationManager.PARAM_CLIENT_ASSERTION_TYPE_VALUE));
        params = params.join("&");
        new WLJSX.Ajax.WLRequest( serverUrl +  WLAuthorizationManager.OAUTH_TOKEN_PATH, {
          ...
          onFailure : function(error){
           WL.Logger.debug('Failed to invoke token with server');
           dfd.reject(error);
          }
          });
          },
...
WLAuthorizationManager.__onTokenSuccess = function (data) {
  var dfd = WLJQ.Deferred();
  var response = data.responseJSON;
  if (response != null) {
     WLAuthorizationManager.__saveTokenByScope(response); // request queue implementation remaining
  }
  dfd.resolve();
  return dfd.promise();
}

WLAuthorizationManager.__onObtainAccessTokenFailure = function (scope, response) {
    var dfd = WLJQ.Deferred();
    if (response.status == 400 && WLAuthorizationManager.shouldCallRegistrationAfterUnknowClientError) {
        WLAuthorizationManager.shouldCallRegistrationAfterUnknowClientError = false;
        WL.Logger.debug("Client instance registration information is incorrect, attempting to re-register client instance.");
        WLAuthorizationManager.__clearRegistration();
        WLAuthorizationManager.__invokeRegistrationRequest().then(
          function (clientId) {
              WLAuthorizationManager.__invokePreAuthRequestWithScopeAndClientId(clientId, scope);
          },
          function (error) {
              dfd.reject(error);
          });
...
WLAuthorizationManager.__getClientIdOrInvokeRegistration = function() {
  var dfd = WLJQ.Deferred();
  ...
  .then(
    ...
    function (error) {
      ...
      var devicedata = JSON.parse(WLAuthorizationManager.__getFromAppSetting(WLAuthorizationManager.CLIENT_REGISTRATION_DATA_KEY));
      if (devicedata.id==WL.StaticAppProps.APP_ID && devicedata.clientPlatform==WL.StaticAppProps.ENVIRONMENT && devicedata.version == WL.StaticAppProps.APP_VERSION && devicedata.hardware == device.manufacturer +' '+ device.model && devicedata.platform == device.platform +' '+ device.version && devicedata.sdk_protocol_version == WLAuthorizationManager.__getSDKProtocolVersion()) {
          dfd.resolve(WLAuthorizationManager.clientId);
      }
      else {
          WLAuthorizationManager.__invokeRegistrationRequest()
         .then(
...
WLAuthorizationManager.__invokeRegistrationRequest = function() {
    var dfd = WLJQ.Deferred();
    var JWSData = {};
    var JWSData = {
        application: {
            id: WL.StaticAppProps.APP_ID,
            clientPlatform: WL.StaticAppProps.ENVIRONMENT,   
            version: WL.StaticAppProps.APP_VERSION
        },
        device: {
            id: WL.Device.getHardwareIdentifier(),
            platform: device.platform +' '+ device.version, 
            hardware: device.manufacturer +' '+ device.model
        },
        attributes: {
            sdk_protocol_version: WLAuthorizationManager.__getSDKProtocolVersion()
        }                  
    };
  if (WLAuthorizationManager.deviceDisplayName != null) {
      JWSData.device.deviceDisplayName = WLAuthorizationManager.deviceDisplayName;
  }
  if  (WLAuthorizationManager.__getFromAppSetting(WLAuthorizationManager.PUBLIC_KEY) == null)
  var privateKey = this.__generateKeyPair();       
  this.__signJWS(JWSData, null).then(
      function (JWSValue) {
      WLAuthorizationManager.__getWlServerUrl().then(
      function(serverUrl) {
        if (serverUrl[serverUrl.length - 1] !== '/') {
        ...
        var parameters = {
            signedRegistrationData: JWSValue       
        };
        parameters = JSON.stringify(parameters);
        parameters = JSON.parse(parameters);
        var relativepath = serverUrl + WLAuthorizationManager.REGISTRATION_PATH;
        var requestmethod = 'POST';
        if (WLAuthorizationManager.clientId != null) {
          WL.Logger.debug('Client exists, application data has changed, calling updateRegistration endpoint');
          requestmethod = 'PUT';
          relativepath += '/' + WLAuthorizationManager.clientId;
        }
            new WLJSX.Ajax.WLRequest( relativepath, {
              method : requestmethod,
              parameters : parameters,
              optionalHeaders : {},
              onSuccess : function(response) {
                if(WLAuthorizationManager.__matchMinimumServerFromResponse(response)){
                 if (response.status == 200) {
                  WL.Logger.debug('Registration update success');
                  dfd.resolve(WLAuthorizationManager.clientId);
                }
                else {
                 WLAuthorizationManager.__onRegistrationSuccess(response).then(
                   function (clientId) {
                ...
                var errorString = "This version of the MobileFirst client SDK requires a minimal server version greater than IFIX " + WLAuthorizationManager.GREATER_THAN_IFIX;
                var responseData = {
                        status: 505, //Represent version not supported
                        responseJSON: {
                            errorCode: "MINIMUM_SERVER",
              ...
              onFailure : function(error) {
                ...
                WLAuthorizationManager.__onRegistrationFailure(error);
                dfd.reject(error);
              }
            });
      },
      ...
      function (error) {
        ...
        failResponse.errorMsg = 'Failed to sign the JWS with following error:' + error;
...
WLAuthorizationManager.__onRegistrationSuccess = function (response) {
    ...
    var data = response.getHeader('Location');
    if (data != null ) {
          var expectedLocationValue = 'registration/clients'; //servercontext work remaning
          if (data.indexOf(expectedLocationValue) > -1)
          {
            var parts = data.split('/');
            this.clientId = parts[parts.length-1];
            WLAuthorizationManager.__storeInAppSetting(WLAuthorizationManager.CLIENT_ID_OAUTH_LABEL, this.clientId);
            this.__setClientRegisteredData();
            dfd.resolve(this.clientId);
          } else {
            var failResponse = new WL.Response(null, null);
            failResponse.errorMsg = 'Registration Response failure - Incorrect Location Header';
            failResponse.errorCode = WL.ErrorCode.UNEXPECTED_ERROR;
            return dfd.reject(failResponse);
          }
    } else {
          var failResponse = new WL.Response(null, null);
          failResponse.errorMsg = 'Registration Response failure - Missing Location Header';
          failResponse.errorCode = WL.ErrorCode.UNEXPECTED_ERROR;
          return dfd.reject(failResponse);
    }
    return dfd.promise();
...
WLAuthorizationManager.__onRegistrationFailure = function (error) {
...
WLAuthorizationManager.__generateKeyPair = function () {
  var cryptography = Windows.Security.Cryptography;
    ...
    try {
        ...
        var privateKey = cryptography.CryptographicBuffer.encodeToBase64String(keypair.export());
        var publicKey = cryptography.CryptographicBuffer.encodeToBase64String(keypair.exportPublicKey());
        this.__storeInAppSetting(WLAuthorizationManager.PRIVATE_KEY, privateKey);
        this.__storeInAppSetting(WLAuthorizationManager.PUBLIC_KEY, publicKey);
    } catch (ex) {
        return null;
    }
    return keypair;
...
WLAuthorizationManager.__signJWS = function (payloadJSON, kid){
  var dfd = WLJQ.Deferred();
  var cryptography = Windows.Security.Cryptography;
  var privateKeyString = this.__getFromAppSetting(WLAuthorizationManager.PRIVATE_KEY);
  var privateKeyBuffer = cryptography.CryptographicBuffer.decodeFromBase64String(privateKeyString);
  var privateKey = cryptography.Core.AsymmetricKeyAlgorithmProvider.openAlgorithm(cryptography.Core.AsymmetricAlgorithmNames.rsaSignPkcs1Sha256).importKeyPair(privateKeyBuffer);
  var publicKey = this.__getFromAppSetting(WLAuthorizationManager.PUBLIC_KEY);
  this.__getModulusAndExponentFromPublicKey(publicKey).then (
      function(result){
        ...
        signedData['header'] = cryptography.CryptographicBuffer.encodeToBase64String(jwsHeader);
        ...
        signedData['payload'] = cryptography.CryptographicBuffer.encodeToBase64String(payload);
        var JWSHeaderAndPayload = WLAuthorizationManager.__getBase64EncodedString(signedData['header']) + '.' + WLAuthorizationManager.__getBase64EncodedString(signedData['payload']);
        var jwsSignature = WLAuthorizationManager.__getBase64EncodedString(WLAuthorizationManager.__signJWSData(cryptography.CryptographicBuffer.convertStringToBinary(JWSHeaderAndPayload, cryptography.BinaryStringEncoding.Utf8), privateKey));          
        signedData['signature'] = jwsSignature;        
        dfd.resolve(signedData);
      },
      function() {
        dfd.reject('Failed to get modulus and exponent for : ' + publicKey);
      }   
  );    
  return dfd.promise();
}

WLAuthorizationManager.__signJWSData = function(data, privateKey) {
  var signature = Windows.Security.Cryptography.Core.CryptographicEngine.sign(privateKey, data);
...
WLAuthorizationManager.__setClientRegisteredData = function() {
    var devicedata = {};
    devicedata['id'] = WL.StaticAppProps.APP_ID;
    devicedata['clientPlatform'] = WL.StaticAppProps.ENVIRONMENT;
    devicedata['version'] = WL.StaticAppProps.APP_VERSION;
    //devicedata['id'] = WL.Device.getHardwareIdentifier();
    devicedata['hardware'] = device.manufacturer +' '+ device.model;
    devicedata['platform'] = device.platform +' '+ device.version;
    devicedata['sdk_protocol_version'] = WLAuthorizationManager.__getSDKProtocolVersion();
    this.__storeInAppSetting(WLAuthorizationManager.CLIENT_REGISTRATION_DATA_KEY,JSON.stringify(devicedata));
}

/* Increase this value whenever a breaking change is introduced in the protocol between server and client.
This number must be agreed upon with other platforms, returns an int representing the protocol version
*/
WLAuthorizationManager.__getSDKProtocolVersion = function() {
  return 1;
}

// Validate that version was sent and it is newer than minimal version. If version is not set - return false.
WLAuthorizationManager.__matchMinimumServerFromResponse = function(response){
  if (WL.Client.isGatewayResponse(response)){
    return true;
  }
  if (response.responseJSON!=null && (WLAuthorizationManager.AUTHENTICATION_SERVER_VERSION_KEY in response.responseJSON)){//did server sent version
    var version = response.responseJSON["server_version"];
    var versionParts = version.split(".");
    if (WLAuthorizationManager.__compareVersions(versionParts,WLAuthorizationManager.MINIMUM_SERVER_SUPPORTED.split("."))<1){
      return true;
    }
    else {
      return false;
...
//      0  if a = b
//      1  if a < b
WLAuthorizationManager.__compareVersions = function(a, b) {
    for (var i = 0; i < 3; i++) {
        ...
        a[i] = WLAuthorizationManager.__addLeadingZeros(a[i]); //add leading zeros to verify string size is equal
        b[i] = WLAuthorizationManager.__addLeadingZeros(b[i]); //add leading zeros to verify string size is equal
    }
...
WLAuthorizationManager.__addLeadingZeros = function(s){
...
WLAuthorizationManager.REGISTRATION_PATH = "registration/v1/self";
WLAuthorizationManager.shouldCallRegistrationAfterUnknowClientError = true;
WLAuthorizationManager.AUTHORIZATION_PATH = "az/v1/authorization";
WLAuthorizationManager.HEART_BEAT_PATH = "preauth/v1/heartbeat";
WLAuthorizationManager.LOGOUT_PATH = "preauth/v1/logout";
WLAuthorizationManager.OAUTH_TOKEN_PATH = "az/v1/token";
WLAuthorizationManager.PRIVATE_KEY = "PRIVATE_KEY";
WLAuthorizationManager.PUBLIC_KEY = "PUBLIC_KEY"
WLAuthorizationManager.CLIENT_ID_OAUTH_LABEL = "com.worklight.oauth.clientid";
WLAuthorizationManager.CLIENT_REGISTRATION_DATA_KEY = "com.worklight.oauth.application.data";
...
WLAuthorizationManager.PARAM_CLIENT_ASSERTION_TYPE_VALUE = "urn:ietf:params:oauth:client-assertion-type:jwt-bearer";
WLAuthorizationManager.authorizationServerUrl = null;
WLAuthorizationManager.heartBeatIntervalInSecs = 420; // 7 minutes
WLAuthorizationManager.heartBeatPeriodicalExecuter = null;
WLAuthorizationManager.deviceDisplayName = null;
WLAuthorizationManager.clientId = null;
WLAuthorizationManager.serverRelativeTime = null;
WLAuthorizationManager.scopetotoken = {};
WLAuthorizationManager.resourceToScopeCache = {};
WLAuthorizationManager.AUTHENTICATION_SERVER_VERSION_KEY = "server_version";
WLAuthorizationManager.MINIMUM_SERVER_SUPPORTED = "8.0.2017020513";
WLAuthorizationManager.GREATER_THAN_IFIX = "8.0.0.0-IF201701250919";
...
/**
 * ================================================================= 
 * Source file taken from :: wlgap.windows8.preview.js
 * ================================================================= 
 */
...
/**
 * OptionsMenu API for the Windows 8 Preview Environment.
 */
__WLOptionsMenu = function() {
    var callbacks = null;
    var optionsMenuDiv;
    ...
    // Workaround for reserved word like reload
    function getConvertedId(id) {
	return 'WLOptionsItem_' + id;
    }

    this.__onItemClicked = function(id) {
	(callbacks[getConvertedId(id)])();
    };
	...
	callbacks = [];
	optionsMenuDiv = document.createElement("div");
	optionsMenu = document.createElement("select");
	var itemToReturn = document.createElement("OPTION");
	itemToReturn.text = "-- Options Menu --";
	itemToReturn.value = "-- Options Menu --";
	itemToReturn.id = "selectOneOptions";
	optionsMenu.options.add(itemToReturn);
	optionsMenu.onchange = function() {
	    var selectedIndex = optionsMenu.options.selectedIndex;
	    if (optionsMenu.options.selectedIndex != 0) {
		optionsMenu.options[selectedIndex].callback();
		optionsMenu.options.selectedIndex = 0;
	    }
	};
	document.body.appendChild(optionsMenuDiv);
	optionsMenuDiv.appendChild(optionsMenu);
	...
	callbacks[getConvertedId(id)] = callback;
	WLJSX.Object.extend(defaultOptions, options);

	var itemToReturn = document.createElement("OPTION");
	itemToReturn.text = title;
	itemToReturn.value = title;
	itemToReturn.id = getConvertedId(id);

	itemToReturn.disabled = !(defaultOptions.enabled);
	itemToReturn.callback = callback;
	optionsMenu.options.add(itemToReturn);
	this.__addWLSettingItem();
	return itemToReturn;
    };
	...
	for ( var i = 0; i < optionsMenu.options.length; i++) {
	    var option = optionsMenu.options[i];
	    if (option.id == getConvertedId(id)) {
		return option;
	    }
	...
	WL.Validators.validateArguments(['string'], arguments, 'WL.OptionsMenu.removeItem');
	for ( var i = 1; i < optionsMenu.options.length; i++) {
	    var option = optionsMenu.options[i];
	    if (option.id == getConvertedId(id)) {
		optionsMenu.remove(i);
	    }
	}
	delete callbacks[getConvertedId(id)];
    };
	...
	while (optionsMenu.options.length > 1) {
	    optionsMenu.remove(1);
	}
	callbacks = [];
	this.__addWLSettingItem();
    };
	...
	WL.Validators.validateArguments(['boolean'], arguments, 'WL.OptionsMenu.setVisible');
	optionsMenu.style.visibility = visible ? "visible" : "hidden";
    };
	...
	return optionsMenu.style.visibility == "visible";
    };
};
...
WL.App.getServerUrl = function(successCallback, failCallback) {
	//For preview we take it from the static  props becuase we do not have a native part:
	successCallback(WL.StaticAppProps.PREVIEW_ROOT_URL);
};

WL.App.setServerUrl = function(url, successCallback, failCallback) {
	//Preview does not support the setServer API because there is no native part
	//So we fail this call always when we are in preview:
	
	failCallback("Setting the server URL is not supported in preview.");
	
};

/**
 * ================================================================= 
 * Source file taken from :: wlauthorizationmanager.preview.js
 * ================================================================= 
 */
...
WLAuthorizationManager.SESSION_STORAGE_PERSISTENCE_POLICY = 'com.worklight.oauth.persistence.policy';
WLAuthorizationManager.SESSION_STORAGE_CLIENT_ID = 'com.worklight.oauth.clientid';
WLAuthorizationManager.SESSION_STORAGE_CLIENT_SECRET = 'com.worklight.oauth.clientsecret';
WLAuthorizationManager.SESSION_STORAGE_ACCESS_TOKEN = 'com.worklight.oauth.accesstoken';
WLAuthorizationManager.SESSION_STORAGE_KEY_ID_TOKEN = 'com.worklight.oauth.idtoken';

WLAuthorizationManager.PARAM_REDIRECT_URI_VALUE = 'http://mfpredirecturi';
WLAuthorizationManager.PARAM_AUTHORIZATION_CODE_VALUE = 'authorization_code';

WLAuthorizationManager.OAUTH_CLIENT_REGISTRATION_PATH = 'authorization/v1/clients/preview';
WLAuthorizationManager.OAUTH_TOKEN_PATH = 'authorization/v1/token';

WLAuthorizationManager.REDIRECT_RESULT_HEADER_NAME = 'Location';

WLAuthorizationManager.authenticationData = {};

//---- manage authentication data and persistence policy -----
WLAuthorizationManager.setAuthorizationPersistencePolicy = function(authorizationPersistencePolicy) {
	...
	if (authorizationPersistencePolicy !== WLAuthorizationManager.__getPersistencePolicy()) {
		// there are only two items to copy, so instead of writing a generic code, which should loop over the storage (which may be large)
		// just handle them in-place
		...
		WLAuthorizationManager.__copyDataAndClear(copyToStorage, WLAuthorizationManager.SESSION_STORAGE_ACCESS_TOKEN);
		WLAuthorizationManager.__copyDataAndClear(copyToStorage, WLAuthorizationManager.SESSION_STORAGE_KEY_ID_TOKEN);
		
		sessionStorage.setItem(WLAuthorizationManager.SESSION_STORAGE_PERSISTENCE_POLICY, authorizationPersistencePolicy);
	}
...
WLAuthorizationManager.__getPersistencePolicy = function() {
	var policy = sessionStorage.getItem(WLAuthorizationManager.SESSION_STORAGE_PERSISTENCE_POLICY);
	
	if (policy === null) {
		sessionStorage.setItem(WLAuthorizationManager.SESSION_STORAGE_PERSISTENCE_POLICY, WLAuthorizationManager.AuthorizationPersistencePolicy.ALWAYS);
...
WLAuthorizationManager.__copyDataAndClear = function(toStorage, key) {
	if (toStorage) {
		sessionStorage.setItem(key, WLAuthorizationManager.authenticationData[key]);
		delete WLAuthorizationManager.authenticationData[key];
	} else {
		WLAuthorizationManager.authenticationData[key] = sessionStorage.getItem(key);
		sessionStorage.removeItem(key);
	}
}

//---- end manage authentication data and persistence policy -----

WLAuthorizationManager.getCachedAuthorizationHeader = function() {
	var dfd = WLJQ.Deferred();
	var accessToken = WLAuthorizationManager.__getAuthDataItem(WLAuthorizationManager.SESSION_STORAGE_ACCESS_TOKEN) || null;
	var idToken = WLAuthorizationManager.__getAuthDataItem(WLAuthorizationManager.SESSION_STORAGE_KEY_ID_TOKEN) || null;
...
WLAuthorizationManager.__getClientInstanceIdHeader = function() {
	...
	var savedClientId = sessionStorage.getItem(WLAuthorizationManager.SESSION_STORAGE_CLIENT_ID);
	if (savedClientId !== null) {
		dfd.resolve(savedClientId);
		return dfd.promise();
	}
	
	WLAuthorizationManager.__getWlServerUrl()
	.then(
		function(serverUrl) {
			serverUrl += WLAuthorizationManager.OAUTH_CLIENT_REGISTRATION_PATH;
			...
			var requestOptions = {
					...
					parameters : {
						...
						environment : WL.StaticAppProps.PREVIEW_ENVIRONMENT
					},
					onSuccess : onSuccess,
					onFailure : onFailure,
			...
			function onSuccess(response) {
				if (typeof(response.responseJSON) === 'undefined' || typeof(response.responseJSON.clientId) === 'undefined') {
					...
					sessionStorage.setItem(WLAuthorizationManager.SESSION_STORAGE_CLIENT_ID, response.responseJSON.clientId);
					sessionStorage.setItem(WLAuthorizationManager.SESSION_STORAGE_CLIENT_SECRET, response.responseJSON.clientSecret);
					dfd.resolve(response.responseJSON.clientId);
				}
			}
			...
			WLAuthorizationManager.__getWlSessionId().then(
				function(sessionId) {
					requestOptions.optionalHeaders[WLAuthorizationManager.WL_X_SESSION_ID_HEADER] = sessionId;
					new WLJSX.Ajax.WLRequest(serverUrl, requestOptions);
				}
			);
...
WLAuthorizationManager.__invokeTokenRequestWithGrantCode = function(grantCode) {
	...
	.then(
		function(serverUrl) {
			WLAuthorizationManager.__sendRequestToTokenEndpoint(serverUrl, grantCode)
			.then(
				function(response) {
					dfd.resolve(response);
				},
...
WLAuthorizationManager.__sendRequestToTokenEndpoint = function(serverUrl, grantCode) {
	var dfd = WLJQ.Deferred();
	
	var requestOptions = {
			method : 'POST',
			parameters : {
				client_id : sessionStorage.getItem(WLAuthorizationManager.SESSION_STORAGE_CLIENT_ID),
	...
	function onSuccess(response) {
		WLAuthorizationManager.__setAuthDataItem(WLAuthorizationManager.SESSION_STORAGE_ACCESS_TOKEN, response.responseJSON.access_token);
		WLAuthorizationManager.__setAuthDataItem(WLAuthorizationManager.SESSION_STORAGE_KEY_ID_TOKEN, response.responseJSON.id_token);
		
		dfd.resolve('Bearer ' + response.responseJSON.access_token + ' ' + response.responseJSON.id_token);
	}
	...
	var authContent = sessionStorage.getItem(WLAuthorizationManager.SESSION_STORAGE_CLIENT_ID) + ':' + 
	sessionStorage.getItem(WLAuthorizationManager.SESSION_STORAGE_CLIENT_SECRET);
											
	WL.SecurityUtils.base64Encode(authContent)
	.then(
		function(authHeader) {
			requestOptions.optionalHeaders['Authorization'] = 'Basic ' + authHeader;
			new WLJSX.Ajax.WLRequest(serverUrl + WLAuthorizationManager.OAUTH_TOKEN_PATH, requestOptions);
		},
		function(error) {
			var failResponse = new WL.Response(null, null);
			failResponse.errorMsg = error;
			failResponse.errorCode = WL.ErrorCode.API_INVOCATION_FAILURE;
			dfd.reject(failResponse);
		}
	);
...
WLAuthorizationManager.__getWlServerUrl = function() {
	var dfd = WLJQ.Deferred();
	dfd.resolve(window.document.location.protocol + '//' + window.document.location.host + '/' +
...
WLAuthorizationManager.__getWlSessionId = function() {
	...
	if (WLAuthorizationManager.__cachedWlSessionId === null || typeof(WLAuthorizationManager.__cachedWlSessionId) === 'undefined') {
		WLAuthorizationManager.__cachedWlSessionId = WLAuthorizationManager.__generateSessionId();
	} 
...
WLAuthorizationManager.__getClientId = function() {
	var dfd = WLJQ.Deferred();
	var clientId = sessionStorage.getItem(WLAuthorizationManager.SESSION_STORAGE_CLIENT_ID);
	dfd.resolve(clientId);
	return dfd.promise();
}

// server ignores signed client id in preview, therefore we can just return non-signed id
WLAuthorizationManager.__getSignedClientId = function() {
	var dfd = WLJQ.Deferred();
	var clientId = sessionStorage.getItem(WLAuthorizationManager.SESSION_STORAGE_CLIENT_ID);
	dfd.resolve(clientId);
	return dfd.promise();
...
WLAuthorizationManager.__deleteAuthData = function() {
	var dfd = WLJQ.Deferred();
	sessionStorage.removeItem(WLAuthorizationManager.SESSION_STORAGE_CLIENT_ID);
	sessionStorage.removeItem(WLAuthorizationManager.SESSION_STORAGE_CLIENT_SECRET);
	WLAuthorizationManager.__removeAuthDataItem(WLAuthorizationManager.SESSION_STORAGE_ACCESS_TOKEN);
	WLAuthorizationManager.__removeAuthDataItem(WLAuthorizationManager.SESSION_STORAGE_KEY_ID_TOKEN);
	WLAuthorizationManager.__cachedWlSessionId = null;
	dfd.resolve();
	return dfd.promise();
}

WLAuthorizationManager.__deleteTokens = function(dfd,response) {
	WLAuthorizationManager.__removeAuthDataItem(WLAuthorizationManager.SESSION_STORAGE_ACCESS_TOKEN);
	WLAuthorizationManager.__removeAuthDataItem(WLAuthorizationManager.SESSION_STORAGE_KEY_ID_TOKEN);
	dfd.resolve(response);
	return dfd.promise();
...
WLAuthorizationManager.__processRedirectResult = function(request, response) {
	var dfd = WLJQ.Deferred();
	var location = response.getResponseHeader(WLAuthorizationManager.REDIRECT_RESULT_HEADER_NAME);
	if (location === null || typeof(location) === 'undefined') {
		dfd.reject(new WL.Response(response, null));
		return dfd.promise();
	}
	
	var result = WLAuthorizationManager.__getParameterByName(location, 'wl_result');
	var jsonResult = JSON.parse(result);
    response.responseJSON = jsonResult;
    
	WL.Client.checkResponseForChallenges(request, response, true);
	
	var grantCode = WLAuthorizationManager.__getParameterByName(location, 'code');
	...
	if (grantCode !== null) {
    	...
    	var error = WLAuthorizationManager.__getParameterByName(location, 'error');
    	WL.Logger.debug('Grant code is null. The error is: ' + error);
...
WLAuthorizationManager.getUserIdentity = function() {
	var dfd = WLJQ.Deferred();
	WLAuthorizationManager.__parseIdTokenToJSON()
	.then(
		function(json) {
			if (json === null) {
				dfd.resolve(null);
			} else {
				dfd.resolve(json['imf.user']);
			}
		},
		dfd.reject
	);
	...
	.then(
		function(json) {
			if (json === null) {
				...
				dfd.resolve(json['imf.application']);
			}
		},
		dfd.reject
	);
...
WLAuthorizationManager.__parseIdTokenToJSON = function() {
	var dfd = WLJQ.Deferred();
	var idToken = WLAuthorizationManager.__getAuthDataItem(WLAuthorizationManager.SESSION_STORAGE_KEY_ID_TOKEN);
	
	if (idToken === null) {
		dfd.resolve(null);
 	} else {
 		var tokenData = idToken.split('.');
		if (tokenData.lenght < 2) {
			dfd.resolve(null);
		} else {
			var data = tokenData[1];
			WL.SecurityUtils.base64Decode(data)
			.then(
				function(response) {
					try {
